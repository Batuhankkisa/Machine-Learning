...
class StressLevelModel:
    def __init__(self, data_path, n_components=None):
    ...
    def preprocess_data(self):
        ...
        if self.n_components:
            ...
            linear_cols_pca = pca.fit_transform(self.df[self.linear_cols])
            self.df = pd.concat([self.df.drop(self.linear_cols, axis=1), pd.DataFrame(linear_cols_pca, columns=[f'PCA_{i}' for i in range(self.n_components)])], axis=1)
            self.linear_cols = [f'PCA_{i}' for i in range(self.n_components)]
    ...
    def evaluate_model(self, model, X_test, y_test, model_type='linear'):
        ...
        if model_type == 'linear':
            ...
            self.plot_scatter_actual_vs_predicted(y_test, y_pred, 'Linear Regression')
    ...
    def plot_roc_curve(self, model, X_test, y_test):
        ...
        plt.plot(fpr, tpr, color='darkorange', lw=2, linestyle='-', label='ROC curve (AUC = {:.2f})'.format(roc_auc))  
...
if __name__ == "__main__":
    ...
    stress_model = StressLevelModel(data_path, n_components=5)  
    stress_model.run()
